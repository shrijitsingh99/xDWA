...
class TrajectoryGraph(nx.DiGraph):
    def __init__(self, costmap,node_num=0):
    ...
    def add_trajectory(self, robot, parent_node_num, depth):
        ...
        for trajectory in self.best_trajectory(trajectories):
            ...
            self.add_edge(parent_node_num, self.node_num, weight=trajectory.score(self.costmap))
    ...
    def generate_trajectory(self, robot):
        generator = SampleGenerator(robot, timestep=0.3, resolution_x=0.05, resolution_theta=0.01)
        vel_samples = generator.generate()
        simulator = TrajectorySimulator(time=5, timestep=0.1)
        trajectories = []
    ...
    def best_trajectory(self, trajectories):
